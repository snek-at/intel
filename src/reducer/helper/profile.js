...
function mergedProfile(){
    ...
    platform.repositories = repositories.map(repository => {
        repository.members = repository.getMembers().map(member => {return member.render()})
        repository.languages = repository.getLanguages().map(language => {return language.render()})
        return repository.render()
    })

    platform.organizations = organizations.map(organization => {
        organization.members = organization.getMembers().map(member => {return member.render()})
        organization.repositories = organization.getRepositories().map(repository => {
            repository.members = repository.getMembers().map(member => {return member.render()})
            repository.languages = repository.getLanguages().map(language => {return language.render()})
            return repository.render()
        })
